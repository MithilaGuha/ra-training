...
This **prior predictive distribution** should be plausible based on your domain expertise and provides a cleaner way to communicate and evaluate the consequences of the assumptions you've made in building your model. We accomplish this by adding the `generated quantities` block in Stan in place of the `parameters` and `model` blocks.
...
We then call the generative model from R and extract the simulated data to perform the prior predictive check.
