...
The workflow is composed of three sections: **model building**, **model calibration**, and **model validation**. The material draws heavily from Michael Betancourt's case studies and training on using Stan at Drexel University in Fall 2018.
...
As part of this conceptual description of the model, we should consider the ideal dataset as well as what summary statistics, including visualizations, we can use to evaluate the model. We encode the information about the observations using the `data` block in Stan.
...
Next, we translate the simplest expression of the conceptual model into a mathematical specification. A fully generative model includes both the likelihood $p(y|\theta)$ (i.e., the data generating process) and the prior $p(\theta)$. We start simple so we can build the model by adding complexity as needed. This is encoded in the `parameters` and `model` blocks in Stan. For example:
...
This **prior predictive distribution** should be plausible based on your domain expertise and provides a cleaner way to communicate and evaluate the consequences of the assumptions you've made in building your model. We accomplish this by using the `generated quantities` block in Stan. For example:
...
We then call the generative model from R and extract the simulated data to perform the prior predictive check. For example:
...
We could also simulate data just using R. However, simulating data using Stan does a few things for us.
